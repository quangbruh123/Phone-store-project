...
const updateWishlist = asyncHandler(async (req, res) => {
	...
	const alreadyHave = user.wishlist.find((el) => el.toString() === pid);
	let updated;
	if (alreadyHave) {
		updated = await User.findByIdAndUpdate(_id, { $pull: { wishlist: pid } }, { new: true });
	} else {
		updated = await User.findByIdAndUpdate(_id, { $push: { wishlist: pid } }, { new: true });
	}
