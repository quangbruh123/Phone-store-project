...
const TrendingCard = ({ product }) => {
    return (
        <Link to={`/product/${product._id}`} className='flex flex-col px-4 py-2 rounded-xl bg-black/[.06] cursor-pointer gap-3'>
            <div className='flex justify-between gap-3 xs:flex-wrap xs:justify-center sm:flex-nowrap sm:justify-between'>
                <div>
                    <h1 className='text-xl xs:text-base sm:text-xl font-bold'>{product.name}</h1>
                </div>
                <div className='flex flex-col items-start '>
                    <div className='flex items-center justify-between'>
                        <h1 className=' text-lg xs:text-base sm:text-lg font-bold'>{product.price} VND</h1>
            ...
            <div className='flex justify-center items-center w-full h-[150px]'>
                <img src={product.image} alt={product.name} className='w-32 h-full py-2 object-contain hover:scale-110 transition' />
            </div>
        </Link>
    );
};
